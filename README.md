...
```bash
git diff --name-only // какие файлы были изменены?

Изменил JSON-задачу      → npm run build:index
Изменил данные или шаблон → npm run build
Изменил JS или CSS        → F5 в браузере
```
